...
def install_asdf_py_essential():
    # asdf 环境需要
    os.system("apt install -y build-essential libssl-dev zlib1g-dev libbz2-dev libreadline-dev libsqlite3-dev curl libncursesw5-dev xz-utils tk-dev libxml2-dev libxmlsec1-dev libffi-dev liblzma-dev")
...
def install_elixir(user):
    os.system("apt install -y elixir inotify-tools")
    user_home_path = os.path.expanduser("~" + user)
    profile = user_home_path + "/" + ".profile"
...
def install_embedded_tools():
    # 编译 openocd 需要的库
    os.system("apt install -y autoconf libtool pkg-config libusb-1.0-0-dev")
    os.system("apt install -y libftdi-dev libhidapi-dev libgpiod-dev")
...
def install_unzipper():
    os.system("apt install -y file-roller")
...
def set_open_extra_menu_alias_to_profile(user):
    user_home_path = os.path.expanduser("~" + user)
    profile = user_home_path + "/" + ".profile"
    file.set_to_file(profile, 'alias open_extra_menu="thunar ~/.local/share/applications"')
...
def do_zsh_config(user):
    ...
    proc.run_as_user(user, cmd)
    file.set_to_file("/etc/zsh/zshenv", 'emulate sh -c "source /etc/profile"', need_new_blank_line=True)
