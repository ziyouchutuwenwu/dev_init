...
create user logical_repl with replication login password '123456';
```
...
grant select on demo_table1 to logical_repl;

或者
grant all privileges on all tables in schema public to logical_repl;

数据库的可以不需要，因为就在当前数据库里面
grant all privileges on database source_db to logical_repl;
```
...
```sh
psql -d dest_db
create subscription demo_subscription connection 'host=192.168.88.100 port=5432 user=logical_repl password=123456 dbname=source_db' publication demo_publication;
```
...
```sh
grant select on demo_table2 to logical_repl;
```
