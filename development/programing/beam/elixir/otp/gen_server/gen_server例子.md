...
defmodule Qps do
  ...
  def check_qps_limitation(ip) do
    status = false

    time = DateTime.utc_now()

    result = QpsGenServer.get_qps_info_by_ip(ip)

    case result do
      {} ->
        QpsGenServer.update_ip_qps_info(ip, {time, 1})
        status
      ...
      {prev, count} ->
        ...
        if elapsed <= 1000 and elapsed > 0 do
          ...
          QpsGenServer.update_ip_qps_info(ip, {time, count + 1})
          status
        end
    end
  end
...
defmodule Demo do
  def demo() do
    ...
    is_limited = Qps.check_qps_limitation(ip)
    IO.inspect(is_limited)
  end
end
```
