...
def patch_link_gnu_lds(flash_size, ram_size):
    ...
    flash_info = (
        "ROM (rx) : ORIGIN = 0x08000000, LENGTH = xxxk => ROM (rx) : ORIGIN = 0x08000000, LENGTH = %sk"
        % flash_size
    )
    ram_info = (
        "RAM (rw) : ORIGIN = 0x20000000, LENGTH = yyyk => RAM (rw) : ORIGIN = 0x20000000, LENGTH = %sk"
        % ram_size
    )
...
def patch_link_keil_sct(flash_size, ram_size):
    ...
    flash_info1 = (
        "LR_IROM1 0x08000000 xxxxxxxx => LR_IROM1 0x08000000 %s" % flash_size_str
    )
    flash_info2 = (
        "ER_IROM1 0x08000000 xxxxxxxx => ER_IROM1 0x08000000 %s" % flash_size_str
    )
