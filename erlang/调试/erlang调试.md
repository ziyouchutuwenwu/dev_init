...
官方插件无法调试，推荐使用[这个 fork 版本](https://github.com/yidayoung/intellij-erlang)，zip 包是可用的版本
...
- 创建一个 erlang 的 shell，或者在 idea 节点里面，创建一个这样的 erlang console

```bash
erl -name aaa@127.0.0.1 -setcookie 111
```

- 创建一个调试用的 remote 节点，一些信息如下配置
...
这个大神的 fork 版可以设置 interpret scope，就是 beam 解析的范围，设置为全工程，则可以下一个断点以后在整体范围内 F7，因为 erlang 的模块默认是单个解析的
